...
internal static class BuildingEntityPatch
{
    ...
    /// <summary>
    /// Re-apply the trap's resolved display name / description after
    /// <c>BuildingEntity.Start</c> runs (which itself overwrites <c>fullName</c>
    /// from <c>Locale.GetBuilding</c>).
    ///
    /// <para>Delegates to <see cref="Utilities.TrapLocale.Resolve"/> so the
    /// lookup order — ScavLib <c>LocaleManager</c> first, legacy
    /// <c>TrapConfig.FullNameCn</c>/<c>DescriptionCn</c> as fallback — stays
    /// identical to <c>TrapBase.SetTrapNameAndDescription</c>. Keeping both
    /// call sites routed through the same resolver prevents the two paths from
    /// drifting (e.g. one honouring a JSON translation while the other still
    /// shows the inlined CN string).</para>
    /// </summary>
    private static void StartPostfix(BuildingEntity __instance)
    {
        ...
        Utilities.TrapLocale.Resolve(trap.Config, out var fullName, out var description);
        __instance.fullName = fullName;
