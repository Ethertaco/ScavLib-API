...
public abstract class TrapBase : MonoBehaviour
{
    ...
    /// <summary>
    /// Apply the trap's display name and description to the live
    /// <see cref="BuildingEntity"/> after its own <c>Start</c> has run.
    ///
    /// <para>Delegates name resolution to <see cref="TrapLocale.Resolve"/>, which
    /// looks up ScavLib's <see cref="ScavLib.i18n.LocaleManager"/>-backed
    /// <c>Locale</c> dictionaries first and falls back to the legacy
    /// <c>TrapConfig.FullNameCn</c> / <c>DescriptionCn</c> branch when no
    /// translation is registered. That ordering lets a downstream trap mod
    /// migrate to JSON / <c>RegisterItem</c> at its own pace without touching
    /// any existing <c>TrapConfig</c> that still inlines its CN strings.</para>
    /// </summary>
    private void SetTrapNameAndDescription()
    {
        if (Config == null || _build == null) return;

        TrapLocale.Resolve(Config, out var fullName, out var description);
        _build.fullName   = fullName;
