...
/// <summary>
/// Multiplayer state helpers — all safe to call without KrokMP installed.
///
/// <para>Soft-dependency convergence: presence detection is owned by
/// <see cref="TrapsModule"/>, which writes the result once into
/// <see cref="TrapLibPlugin.KrokMpEnabled"/>. Every method here short-circuits
/// on that flag before paying the cost of a reflection lookup, so there is a
/// single source of truth for "is KrokMP loaded" — and it lives next to ScavLib's
/// own <c>KrokMpBridge</c> detection (both use the same
/// <c>Chainloader.PluginInfos</c> probe at boot time).</para>
/// </summary>
public static class MPSync
{
    ...
    /// <summary>
    /// True when this instance is a pure KrokMP client (network up, not server).
    /// Always false when KrokMP is not installed — short-circuits on
    /// <see cref="TrapLibPlugin.KrokMpEnabled"/> to avoid paying for reflection
    /// every time something on the trap hot path asks.
    /// </summary>
    public static bool IsClient
    {
        get
        {
            // ── Convergence point: trust TrapsModule's Chainloader probe. ──
            if (!TrapLibPlugin.KrokMpEnabled) return false;
            ...
            catch (Exception ex)
            {
                TrapLibPlugin.Log?.LogWarning(
                    $"[MPSync] IsClient reflection failed (KrokMP API may have changed): {ex.Message}");
                return false;
            }
        }
    }
    ...
    /// In multiplayer uses <c>NetPlayer.AllLivingPlayers</c> (via reflection);
    /// in single-player falls back to <c>PlayerCamera.main.body</c>.
    /// Safe to call without KrokMP installed — degrades to the SP path.
    /// </summary>
    public static IEnumerable<Body> AllPlayerBodies
    {
        get
        {
            // SP fast path: skip reflection entirely when KrokMP isn't loaded.
    ...
    {
        // Two-layer gate: pure-client check first (cheap), then plugin-presence.
        if (IsClient) return;                       // Server/SP only.
        if (!TrapLibPlugin.KrokMpEnabled) return;   // No KrokMP → nothing to sync.
        ...
        if (_forceSync == null)
        {
            ...
            {
                TrapLibPlugin.Log?.LogWarning(
                    $"[MPSync] QueueHealthSync setup failed (KrokMP API may have changed): {ex.Message}");
                _forceSync = _ => { };
            }
